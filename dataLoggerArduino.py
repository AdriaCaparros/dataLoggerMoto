...
while (switch_state==True):
    ...
    if message is not None:
        valor=0
        if(message.arbitration_id==0x02F):
            ...
            message = bus.recv(timeout=1.0)
            while((message is None) or(not message.arbitration_id==0x274)):
                message = bus.recv(timeout=1.0)
            print("Id: ",message.arbitration_id)
            ...
            message = bus.recv(timeout=1.0)
            while ((message is None) or (not message.arbitration_id == 0x195)):
                message = bus.recv(timeout=1.0)
            print("Id: ", message.arbitration_id)
            ...
            message = bus.recv(timeout=1.0)
            while ((message is None) or (not message.arbitration_id == 0x146)):
                message = bus.recv(timeout=1.0)
            print("Id: ", message.arbitration_id)
            ...
            c += '{0:f},{1:f},{2:f},{3:f},{4:f},{5:f},{6:f},{7:f},{8:f},{9:f},{10:f},{11:f}'.format(voltatge,corrent,corrent_maxim,temperatura_motor,temperatura_inversor,velocitat,revolucions,accelerador,temperatura1,temperatura2,temperatura3,temperatura4)
            print('\r {} '.format(c))
